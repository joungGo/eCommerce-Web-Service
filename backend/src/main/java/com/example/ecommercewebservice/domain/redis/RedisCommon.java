...
public class RedisCommon {
    ...
    // redis 에서 데이터 받아오기 - 캐시된 데이터를 객체로 조회
    ...
    public <T> void setDataWithTTL(String key, T value, Duration expiredTime) {
        String jsonValue = gson.toJson(value);
        template.opsForValue().set(key, jsonValue, expiredTime.toSeconds());
    }

    // redis 에 여러 데이터를 한번에 저장하기 - 대량 데이터 저장
...
//            return clazz.cast(result);
    ...
    // Redis Hash에서 특정 필드를 삭제.
    public void removeFromhash(String key, String field) {
        template.opsForHash().delete(key, field);
    }

    /*// 키의 값과 남은 TTL을 함께 조회. - 캐시 유효 기간 확인.
    public <T> ValueWithTTL<T> GetValueWithTTl(String key, Class<T> clazz) { // 값과 TTL을 함께 반환        T value = null;                                                       // 반환할 값 초기화        Long ttl = null;                                                      // TTL 초기화
        try {            List<Object> results = template.executePipelined(new RedisCallback<Object>() { // 파이프라인으로 다중 명령 실행                public Object doInRedis(RedisConnection connection) throws DataAccessException {                    StringRedisConnection conn = (StringRedisConnection) connection; // 문자열 기반 연결로 캐스팅                    conn.get(key);                                           // 키의 값 조회                    conn.ttl(key);                                           // 키의 남은 TTL(초 단위) 조회                    return null;                                             // 파이프라인에서는 null 반환                }            });
            value = (T) gson.fromJson((String) results.get(0), clazz);       // 첫 번째 결과(값)를 JSON 역직렬화            ttl = (Long) results.get(1);                                     // 두 번째 결과(TTL) 할당        } catch (Exception e) {            e.printStackTrace();                                             // 예외 발생 시 스택 출력        }
